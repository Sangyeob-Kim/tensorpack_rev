    ...
    def _build_graph(self, inputs):
        ...
        with remap_variables(new_get_variable), \
            ...
            logits = (LinearWrap(image)
                      .Conv2D('conv0', 96, 12, strides=4, padding='VALID', split=2, after=16)
                      .tf.identity(name = 'conv0_output')
                      .apply(activate)
                      .Conv2D('conv1', 256, 5, padding='SAME', split=2, after=16)
                      .tf.identity(name = 'conv1_output')
                      .apply(fg)
                      .BatchNorm('bn1')
                      .MaxPooling('pool1', 3, 2, padding='SAME')
                      .apply(activate)

                      .Conv2D('conv2', 384, 3, split=2, after=16)
                      .tf.identity(name = 'conv2_output')
                      .apply(fg)
                      .BatchNorm('bn2')
                      .MaxPooling('pool2', 3, 2, padding='SAME')
                      .apply(activate)

                      .Conv2D('conv3', 384, 3, split=2, after = 16)
                      .tf.identity(name = 'conv3_output')
                      .apply(fg)
                      .BatchNorm('bn3')
                      .apply(activate)

                      .Conv2D('conv4', 256, 3, split=2, after = 16)
                      .tf.identity(name = 'conv4_output')
                      .apply(fg)
                      .BatchNorm('bn4')
